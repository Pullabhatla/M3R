...
for trial in range(1, 4):
    ...
    start = time()
    history1 = mlp1.rmsprop_train(x_train, y_train, 1e-1, 1000, 1e-7, 0.9, 32, x_test, y_test)
    end = time()
    ...
    plt.xlabel('Epoch')
    plt.legend()
    plt.savefig('figures/' + str(trial) + 'lrmse-1.png')
    plt.clf()
    ...
    plt.xlabel('Epoch')
    plt.legend()
    plt.savefig('figures/' + str(trial) + 'armse-1.png')
    plt.clf()

    mlp2 = LeakyReLUSoftmaxCCE((28, 28), 10, [16, 16, 16])

    start = time()
    history2 = mlp2.rmsprop_train(x_train, y_train, 1e-3, 1000, 1e-7, 0.9, 32, x_test, y_test)
    end = time()
    ...
    plt.xlabel('Epoch')
    plt.legend()
    plt.savefig('figures/' + str(trial) + 'lrmse-3.png')
    plt.clf()
    ...
    plt.xlabel('Epoch')
    plt.legend()
    plt.savefig('figures/' + str(trial) + 'armse-3.png')
    plt.clf()

    mlp3 = LeakyReLUSoftmaxCCE((28, 28), 10, [16, 16, 16])

    start = time()
    history3 = mlp3.rmsprop_train(x_train, y_train, 1e-5, 1000, 1e-7, 0.9, 32, x_test, y_test)
    end = time()
    ...
    plt.xlabel('Epoch')
    plt.legend()
    plt.savefig('figures/' + str(trial) + 'lrmse-5.png')
    plt.clf()
    ...
    plt.xlabel('Epoch')
    plt.legend()
    plt.savefig('figures/' + str(trial) + 'armse-5.png')
    plt.clf()

with open('output/rmsprop.txt', 'w') as f:
    f.write('\n'.join(lines))
