...
for trial in range(1, 4):
    ...
    start = time()
    history1 = mlp1.adagrad_train(x_train, y_train, 1e-1, 1000, 1e-7, 32, x_test, y_test)
    end = time()
    ...
    plt.xlabel('Epoch')
    plt.legend()
    plt.savefig('figures/' + str(trial) + 'ladge-1.png')
    plt.clf()
    ...
    plt.xlabel('Epoch')
    plt.legend()
    plt.savefig('figures/' + str(trial) + 'aadge-1.png')
    plt.clf()

    mlp2 = LeakyReLUSoftmaxCCE((28, 28), 10, [16, 16, 16])

    start = time()
    history2 = mlp2.adagrad_train(x_train, y_train, 1e-3, 1000, 1e-7, 32, x_test, y_test)
    end = time()
    ...
    plt.xlabel('Epoch')
    plt.legend()
    plt.savefig('figures/' + str(trial) + 'ladge-3.png')
    plt.clf()
    ...
    plt.xlabel('Epoch')
    plt.legend()
    plt.savefig('figures/' + str(trial) + 'aadge-3.png')
    plt.clf()

    mlp3 = LeakyReLUSoftmaxCCE((28, 28), 10, [16, 16, 16])

    start = time()
    history3 = mlp3.adagrad_train(x_train, y_train, 1e-5, 1000, 1e-7, 32, x_test, y_test)
    end = time()
    ...
    plt.xlabel('Epoch')
    plt.legend()
    plt.savefig('figures/' + str(trial) + 'ladge-5.png')
    plt.clf()
    ...
    plt.xlabel('Epoch')
    plt.legend()
    plt.savefig('figures/' + str(trial) + 'aadge-5.png')
    plt.clf()

with open('output/adagrad.txt', 'w') as f:
    f.write('\n'.join(lines))
